    ...
    public partial class YoneticiGiris : Form
    {
        ...
        private void buttongirisyap_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string query = "SELECT AdminID FROM Admin WHERE AdminName = @AdminName AND Password = @Password";

                using (SqlCommand command = new SqlCommand(query, VeriTabaniBaglantisi.baglanti))
                {

                    command.Parameters.AddWithValue("@AdminName", isim);
                    ...
                    if (result != null)
                    {
                       ...
                       YoneticiSayfasi yoneticiSayfasi = new YoneticiSayfasi(adminID);
